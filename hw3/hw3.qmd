...
```{r}
pal <- c("asian_pop" = "#560bad", 
         "black_pop" = "#8BC34A",
         "indigenous_pop" = "#AD1457",
         "white_pop" = "#F44336", 
         "latino_pop" = "#009688",
         "other_race" = "#F9C74F",
         "two_races" = "#FF9800",
         "paci_islander_pop" = "#1565C0")

pop_label <- c("asian_pop" = "Asian",
                "black_pop" = "Black",
               ...
               "paci_islander_pop" = "Pacific Islander")
```
...
very_high <- bp_ethnicity_nri |> 
  ...
  arrange(values) |> 
ggplot(aes(fill= fct_reorder(variable, values, .desc = FALSE), 
           y=values, 
           x=county_name)) + 
...
relatively_low <- bp_ethnicity_nri |> 
 ...
 arrange(values) |> 
ggplot(aes(fill= fct_inorder(variable),
           y=values, 
           x=county_name)) +
    geom_bar(position="fill", stat="identity") +
  coord_flip() +
...
```{r}
#| fig-alt: There are four stacked bar plots showing the ethnic break down of the populations of California counties. Each stacked bar plot is all of the counties in each NRI category represented in California, either "Very High", "Relatively High", "Relatively Moderate", or "Relatively Low". Counties with NRI scores in the Very High and Relatively High risk categories have a higher proportion of Latino population than counties with NRI scores in the Relatively Moderate and Relatively Low risk categories. There is only one county California in the "Relatively Low" NRI category" 
...
very_high +  relatively_high  + relatively_moderate + relatively_low +
  plot_annotation(title = "Population proportions by race and ethnicity\ndivided by NRI score risk categories for counties in California",
                  subtitle = "Population proportions are ordered with the ethnicity or race with the highest proportion for that NRI score category on the left.\nCounties with NRI scores in the Very High and Relatively High risk categories have a higher proportion of Latino population\nthan counties with NRI scores in the Relatively Moderate and Relatively Low risk categories",
...
**3. Summarize your main finding in no more than two sentences.**  
 Counties with NRI scores in the Very High and Relatively High risk categories have a higher proportion of Latino population than counties with NRI scores in the Relatively Moderate and Relatively Low risk categories
