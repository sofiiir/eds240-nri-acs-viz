...
```{r}
hex_nri <- full_join(hexagons, nri_avg_hex, by = join_by(state_abbr))
```
...
```{r}
ggplot(hex_nri) + 
  geom_sf(aes(fill = nri_perc_mean)) + 
  geom_sf_text(aes(label = state_abbr), color = "white", size = 3, alpha = 0.6) +
  scale_fill_gradient() +
  theme_void() +
